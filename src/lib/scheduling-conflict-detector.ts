...
// ---------------------------------------------------------------------------
// HARD scheduling conflict — fires a fixed handoff + AI pause
// ---------------------------------------------------------------------------
// The patterns above catch SOFT signals (operator review, AI keeps going).
// The pattern below catches UNAMBIGUOUS "your booking widget literally
// can't accommodate me" statements — at which point the AI must stop
// generating its own scheduling guesses, ship a single fixed handoff
// message, pause the conversation, and escalate to a human. This is the
// failure mode where the AI cheerfully confirms "Saturday works!" right
// after the lead said "I can't do this weekend".
// ---------------------------------------------------------------------------

const HARD_SCHEDULING_CONFLICT_RE =
  /\b(won'?t let me (select|choose|pick)|only (shows?|has?|gives?) (saturday|sunday|monday|one day)|not available (this|that|next) (weekend|week|day)|can'?t (make|do) (any of those|that day|this weekend)|none of (those|these) (work|times)|no (other|available) (days?|times?|slots?))\b/i;

/**
 * Strict variant. Matches only language that explicitly states the
 * available slots cannot accommodate the lead — no fuzzy day-mention
 * or counter-proposal heuristics. False positives must be near-zero
 * because firing this triggers a fixed-text handoff and pauses the AI
 * for the rest of the conversation.
 */
export function detectHardSchedulingConflict(text: string): {
  detected: boolean;
  match: string | null;
} {
  if (typeof text !== 'string' || text.trim().length === 0) {
  ...
  const m = text.match(HARD_SCHEDULING_CONFLICT_RE);
  return m ? { detected: true, match: m[0] } : { detected: false, match: null };
}

/**
 * Fixed handoff message. Used verbatim when detectHardSchedulingConflict
 * fires — never paraphrased, never regenerated by the LLM. Stable copy
 * keeps the lead-facing voice consistent across every conflict event
 * and gives ops a known phrase to grep for in logs.
 */
export const HARD_SCHEDULING_HANDOFF_MESSAGE =
