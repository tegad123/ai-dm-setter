  ...
  if (
    ...
    try {
      ...
      const outboundBridgeReference = opener
        ...
        : `the ${downsellProductName}`;
      // B5: when ManyChat already asked the experience/background question, the
      // "start with trading background" bridge would contradict the dedup. Skip
      // straight to the next stage (goal) and lead the dedup instruction so it
      // overrides the discovery-bridge framing AND the few-shot examples that
      // otherwise steer the model back into re-asking experience.
      const bridgeLine = manyChatNativeQuestion
        ? `Natural bridge: ManyChat's automation ALREADY asked this lead "${manyChatNativeQuestion}" and their latest message is the ANSWER to it. Do NOT re-ask it or any paraphrase (do not ask "are you new / how long have you been trading" — that is answered). Acknowledge their answer in one line, then move to the GOAL question (what they want from trading). Ignore any example or instruction that opens by asking about trading background/experience — that step is already done.`
