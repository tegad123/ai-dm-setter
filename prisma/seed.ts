...
Analyze the conversation history and return ONLY a number from 0 to 100.`,
  ...
  const leadsData = [
    ...
    },
    {
      ...
      platform: 'INSTAGRAM' as const,
      status: 'TRUST_OBJECTION' as const,
      qualityScore: 65,
      triggerType: 'COMMENT' as const
    },
    {
      ...
      platform: 'INSTAGRAM' as const,
      status: 'IN_QUALIFICATION' as const,
      qualityScore: 72,
      triggerType: 'DM' as const
    },
    {
    ...
    },
    {
      ...
      handle: '@jordan.lee',
      platform: 'FACEBOOK' as const,
      status: 'MONEY_OBJECTION' as const,
      qualityScore: 55,
      triggerType: 'DM' as const
    },
    {
    ...
    },
    {
      ...
      handle: '@ethan.wright',
      platform: 'INSTAGRAM' as const,
      status: 'SERIOUS_NOT_READY' as const,
      qualityScore: 70,
      triggerType: 'COMMENT' as const
    },
    {
  ...
  for (let i = 0; i < genericConversations.length; i++) {
    ...
    const convo = await prisma.conversation.create({
      data: {
        leadId: lead.id,
        aiActive: lead.status !== 'TRUST_OBJECTION',
        unreadCount: lead.status === 'NEW_LEAD' ? 1 : 0,
