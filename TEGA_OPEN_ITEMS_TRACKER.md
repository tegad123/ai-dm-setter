...
## SMOKE-TEST FOLLOW-UPS (2026-08-19, Tega Test-4 + P1s)

- TEST 4 (canSend didn't block a held conversation): ROOT-CAUSED + FIXED, verified on prod (Shazim FB conv cmsqhobkb, authoritative on): held+automated→BLOCKED (HELD_GATE_EXHAUSTED), held+operator→ALLOWED, active→ALLOWED. Two bugs: (1) `2150f11` thread the exact conversationId to the gate (was guessing via platformUserId→latest-conversation heuristic; wrong conv for a lead with multiple); (2) `4e56cc9` decouple enforcement from the shadow-log write (an intermittent PrismaClientInitializationError on the log create() threw into the outer catch and failed the gate OPEN — allowing a send the verdict said to block). Log is now best-effort; enforcement returns the verdict regardless.
- P1-B + exhaustion-to-hold: on the low-ticket "script-is-product" funnel, soft-unshippable gate exhaustion now HOLDS for review + fires an IMMEDIATE notification (escalate ai_stuck), instead of shipping best-effort. Closes banned_emoji-ships-on-turn-1 and the exhaustion-ships-flagged-content class together. High-ticket personas keep best-effort ship (forward reply beats silence there). `<pending commit>`.
- ManyChat egress honesty (Tega asked to state it explicitly): ManyChat automations deliver openers DIRECTLY via ManyChat's own send, never touching our send functions (sendDM/sendMessage), so canSend CANNOT gate them. "Single egress gate" is true for AI-originated sends only. A held or AI-off conversation CAN still be messaged by a ManyChat automation. Structural fix (route ManyChat sends through our gate, or mirror hold-state to ManyChat) is out of Phase 0 scope — flagged, not silently assumed.
- STILL COMPILER-CLASS (Tega's own argument — "under a compiled FSM P1-A cannot exist"): P1-A recovery path (re-fire ask when a step parks on waitable_message_not_found), branch-misroute duplicate-send, step-attribution drift. These are generation-layer, upstream of canSend — patching them is the prose-on-prose gate-stacking the compiler replaces. Scoped to the compiler (M5), not patched now.
- NOT CODE: HUMAN_AGENT not approved on the Meta app → all >24h follow-ups fail at Meta (Meta app-review action, biggest silent revenue hole); ManyChat attribution end-to-end (is the External Request firing on the live flow? partly Daniel's ManyChat config). Both flagged for owner action.
- Dashboard gaps (M5 UI): no operator control to FLAG a conversation for review (only resolve holds); no shareable per-conversation URL. Queued.
