// Resets Seemal's low-ticket test conversation to a clean step-1 slate so the
...
const SENDER_ID = '27377136925304709'; // Seemal
...
async function main() {
  ...
  if (!lead) throw new Error('no Seemal lead found');
  ...
  if (!conv) throw new Error('no Seemal conversation found');
  console.log(
