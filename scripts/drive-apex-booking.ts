// Apex Trades / Marcus Rivera — booking path E2E funnel.
// Lead persona has capital ABOVE threshold (£3,000) so R24 passes and the
// conversation should reach BOOKED with a Calendly link, no stalls.
//
// Usage:
//   npx tsx scripts/drive-apex-booking.ts            # run the funnel
//   npx tsx scripts/drive-apex-booking.ts --cleanup  # delete the test lead
...
// Lead persona: capital £3,000 — well above threshold so R24 passes and the
// booking Calendly link is delivered (stage=BOOKED).
...
- been trading ~6 months, inconsistent results, want a proper system
- based in Manchester, UK
- biggest problem: no clear entry rules, end up chasing trades
- you work in finance, take home about £4,500/month, want trading as a side income
- income goal: £3,000/month from trading on top of your salary
- deeper why: want financial freedom and to not be tied to one employer
- main obstacle: no structure, self-taught, keep second-guessing entries
- you're motivated and have proper capital to invest in yourself
- capital: you have £3,000 set aside specifically for this right now
- when asked about capital be clear and confident: "yeah I've got about £3k I can put into this"
- when offered a strategy call or booking link: agree readily, you're keen to get started
- when they send a Calendly link: say you'll book it now and ask what to expect on the call
...
function snapshot(st: Awaited<ReturnType<typeof getState>>): string {
  ...
  return `[stage=${st?.lead.stage} sys=${st?.convo?.systemStage} step=${st?.convo?.currentScriptStep} capital=${cdp?.verifiedCapitalUsd?.value ?? '-'} thresholdMet=${cdp?.capitalThresholdMet?.value ?? '-'} panel=[${lit.join(',')}]]`;
}
...
function verifyTurn(params: {
  ...
  if (cdp?.capitalThresholdMet?.value === true) {
    ok('capitalThresholdMet=true — R24 passed, booking path active');
  } else if (cdp?.capitalThresholdMet?.value === false) {
    bad(
      'capitalThresholdMet=false — unexpected downsell path for this persona'
    );
  }
...
async function main() {
  ...
  console.log(
    `\n=== APEX TRADES BOOKING PATH E2E (page ${ENTRY_ID}, sender ${SENDER_ID}) ===`
  );
  console.log(
    `=== Persona: UK lead, Manchester, capital £3,000 (above threshold) ===\n`
  );

  let nextLeadMsg =
    'hey man seen your content on trading, been at it myself but struggling to get consistent';
  ...
  for (let turn = 1; turn <= MAX_TURNS; turn++) {
    ...
    // Terminal: Calendly link sent (booking link in message)
    if (
      fresh.some((r) =>
        /calendly\.com|cal\.com|book.*call|schedule.*call|grab.*time|pick.*time/i.test(
          r
        )
      )
    ) {
      console.log(`\n🎉 BOOKING LINK DELIVERED — ${snapshot(st)}`);
      break;
    }
    if (fresh.length === 0) {
    ...
    for (let attempt = 1; attempt <= 4 && !replyText; attempt++) {
      const result = await callHaikuText({
        ...
        temperature: 0.7,
        timeoutMs: 30000,
        logPrefix: `[apex-book t${turn} a${attempt}]`
      });
